...
export class ConnectorPointsOrthogonalWestSideCalculator extends ConnectorPointsOrthogonalSideCalculatorBase {
    ...
    getScaleableAsideOffsetPoints(originPoint: ConnectorRenderPoint, targetPoint: ConnectorRenderPoint, originRect: Rectangle, isEnd?: boolean, direction?: boolean): ConnectorRenderPoint[] {
        const offset = this.getScaleableOffsetX(originPoint, isEnd);
    ...
    getScaleableOffsetX(originPoint: ConnectorRenderPoint, isEnd: boolean): number {
        if(this.beginRect && this.endRect) {
            const distance = isEnd ? originPoint.x - this.beginRect.right : originPoint.x - this.endRect.right;
    ...
    isDirectConnectionAllowed(targetSide: ConnectionPointSide, originPoint: ConnectorRenderPoint, targetPoint: ConnectorRenderPoint): boolean {
        return targetSide === ConnectionPointSide.East || targetSide === ConnectionPointSide.Undefined;
    }
    getDirectConnectionPoints(originPoint: ConnectorRenderPoint, targetPoint: ConnectorRenderPoint): ConnectorRenderPoint[] {
        const cx = targetPoint.x + (originPoint.x - targetPoint.x) / 2;
        return [
