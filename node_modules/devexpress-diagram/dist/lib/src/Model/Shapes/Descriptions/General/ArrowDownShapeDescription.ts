...
export class ArrowDownShapeDescription extends ArrowVerticalShapeDescription {
    get key() { return ShapeTypes.ArrowDown; }
    ...
    createShapePrimitives(shape: Shape): SvgPrimitive<SVGGraphicsElement>[] {
        ...
        const p0dy = height - shape.parameters.get(ArrowVerticalTriangleHeightParameterName).value;
        ...
        return [
            new PathPrimitive([
                new PathPrimitiveMoveToCommand(p1x1, top),
                new PathPrimitiveLineToCommand(p1x2, top),
        ...
        return [
            new ShapeParameterPoint("c",
                new Point(
                    ...
                    shape.normalizeY(shape.position.y + shape.size.height - shape.parameters.get(ArrowVerticalTriangleHeightParameterName).value)
                )
            )
        ];
    }
    processConnectionPoint(shape: Shape, point: ConnectionPoint) {
        ...
        if(point.y > shape.position.y + shape.size.height - triangleHeight) {
            ...
            const delta = (point.y - (shape.position.y + shape.size.height - triangleHeight)) * tg;
